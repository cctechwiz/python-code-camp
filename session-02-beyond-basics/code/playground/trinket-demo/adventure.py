...
class Game(Base):
  ...
  def run(self , update_func = False):
    ...
    while True:
      ...
      # first check phrases
      things = list(actor.inventory.values()) + list(actor.location.contents.values()) + list(actor.location.actors) + [actor.location] + [actor]
      done = False
      for thing in things:
...
class Actor(Base):
  ...
  def act_list_verbs( self, actor, noun, words ):
    ...
    self.output( (lambda x: x)(" ".join(sorted(self.verbs.keys()))), FEEDBACK )
    return True
